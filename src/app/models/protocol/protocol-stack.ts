...
import { isPageInterface } from '../../guards/type.guard';
...
export class ProtocolStack {
  ...
  readonly currentPageObservable: Observable<PageInterface> = this.latestProtocolObservable.pipe(
    map(item => (item ? item.pageQueue.at(item.pageIndex) : undefined)),
    filter((page): page is PageInterface => {
      return page !== undefined && isPageInterface(page);
    })
  );
  ...
  addProtocol(protocol: ProtocolInterface | ProtocolSchemaInterface): void {
    ...
    const item: ProtocolStackItem = {
      ...
      startTime: new Date(),
      maxSeconds: protocol.timeout?.nMaxSeconds ?? Number.MAX_SAFE_INTEGER,
      maxPages: protocol.timeout?.nMaxPages ?? protocolCopy.pages.length,
