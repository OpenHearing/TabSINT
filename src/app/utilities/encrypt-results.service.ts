...
export class EncryptResultsService {
   ...
   */
  async encryptForUpload(testDateTime: string, uuid: string, publicKeyPem: string, plaintext: string): Promise<[string, string]> {
    const key = await this.deriveKey(testDateTime, uuid);
    const iv = crypto.getRandomValues(new Uint8Array(16));
    const ciphertext = await crypto.subtle.encrypt({ name: 'AES-CBC', iv }, key, new TextEncoder().encode(plaintext));
    const combined = new Uint8Array(16 + ciphertext.byteLength);
  ...
  private async importPublicKey(pem: string): Promise<CryptoKey> {
    ...
    return crypto.subtle.importKey('spki', buffer, { name: 'RSA-OAEP', hash: 'SHA-256' }, false, ['encrypt']);
  }
}
