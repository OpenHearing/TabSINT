...
export class MemrExamComponent implements OnInit, OnDestroy {
  ...
  getRecordedLevelLeq(index: number): string {
    if (
      this.memrResults?.RecordedLeq_dBSPL &&
  ...
  private initializeResponseArea(responseArea: MemrExamInterface): void {
    ...
    this.trialsPerBlock =
      (this.isWithinBlock() ? this.memrExamProperties.elicitorLevelArray?.length : this.memrExamProperties.nRepeats) ?? this.trialsPerBlock;
    this.blockCount =
      (this.isWithinBlock() ? this.memrExamProperties.nRepeats : this.memrExamProperties.elicitorLevelArray?.length) ?? this.blockCount;
    ...
    this.inputParameterMap = new Map([
      ...
      [
        ...
        JSON.stringify(this.memrExamProperties.elicitorOutputChannel ?? memrSchema.properties.elicitorOutputChannel.default),
      ],
