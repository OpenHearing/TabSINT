...
export class WAIResultsComponent implements AfterViewInit {
  ...
  private createResultsPlot() {
    ...
    graphs.forEach(({ id, x, y, w, h, xRange, yRange, yAxisFormat, data }, index) => {
      xScale = d3.scaleLog().domain(xRange).range([0, plotWidth]);
      yScale = d3.scaleLinear().domain(yRange).range([plotHeight, 0]);
      ...
      defs.append('clipPath').attr('id', clipId).append('rect').attr('x', 0).attr('y', 0).attr('height', h).attr('width', w);

      // Apply transformations and clipping to the group for additional plotting steps
      const transformedClipGroup = svg.append('g').attr('clip-path', `url(#${clipId})`).attr('transform', `translate(${x},${y})`);
      ...
      if (id == 'Absorbance') {
        ...
        transformedClipGroup.append('path').attr('d', normativeAbsorbancePath).attr('fill', 'gray').attr('stroke', 'gray').attr('stroke-width', 2);
        ...
        transformedClipGroup.append('path').attr('d', normativeReflectancePath).attr('fill', 'gray').attr('stroke', 'gray').attr('stroke-width', 2);
      }
