...
import { ChaWrap } from 'capacitor-plugin-creare-cha';
...
export class ConfigComponent {
  ...
  async capacitorPluginTest() {
    const { value } = await ChaWrap.echo({ value: 'Hello World!' });
    console.log('Response from native:', value);

    const { result } = await ChaWrap.execute({ cmd: 'echo',args:['list','of','strings'] });
