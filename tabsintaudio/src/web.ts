...
import type { ConfigureOptions, PreloadOptions, TabsintAudioPlugin } from './definitions';

export class TabsintAudioWeb extends WebPlugin implements TabsintAudioPlugin {
  ...
  async preload(options: PreloadOptions): Promise<void> {
    if (TabsintAudioWeb.AUDIO_ASSET_BY_ASSET_ID.has(options.assetId)) {
      throw 'AssetId already exists. Unload first if like to change!';
    }
    if (!options.assetPath?.length) {
      throw 'no assetPath provided';
    }
    if (!options.isUrl && !new RegExp('^/?' + TabsintAudioWeb.FILE_LOCATION).test(options.assetPath)) {
      const slashPrefix: string = options.assetPath.startsWith('/') ? '' : '/';
      options.assetPath = `${TabsintAudioWeb.FILE_LOCATION}${slashPrefix}${options.assetPath}`;
    }
    ...
    TabsintAudioWeb.AUDIO_ASSET_BY_ASSET_ID.set(options.assetId, new AudioAsset(audio));
  }
  ...
  async unload(options: { assetId: string }): Promise<void> {
    await this.stop(options);
    TabsintAudioWeb.AUDIO_ASSET_BY_ASSET_ID.delete(options.assetId);
  }
  ...
  async setSystemVolume(options: { volume: number }): Promise<void> {
  ...
  private getAudioAsset(assetId: string): AudioAsset {
    ...
    if (!TabsintAudioWeb.AUDIO_ASSET_BY_ASSET_ID.has(assetId)) {
      throw `no asset for assetId "${assetId}" available. Call preload first!`;
    }

    return TabsintAudioWeb.AUDIO_ASSET_BY_ASSET_ID.get(assetId) as AudioAsset;
  }
